   ...
   do
    ...
    do
      ...
      Tabs.Combat:AddToggle("MobileFly",{Title="Fly",Default=false,Callback=function(v)
       ...
       Tabs.Extras:AddParagraph({Title="Invisibility 2", Content="Rejoin to disable"})
        Tabs.Extras:AddButton({Title="Apply Invisibility 2", Callback=function()
            task.spawn(function()
                repeat task.wait() until game:IsLoaded()
                local Players = game:GetService("Players")
                local LocalPlayer = Players.LocalPlayer
                local Camera = workspace.CurrentCamera
                local NOTIFY_URL = "https://raw.githubusercontent.com/IceMinisterq/Notification-Library/Main/Library.lua"
                pcall(function()
                    loadstring(game:HttpGet(NOTIFY_URL))():SendNotification("Success","your a invis now",3)
                end)
                local INVIS_ANIM = "rbxassetid://119635500264882"
                local function makeInvisible(character)
                    if not character then return end
                    local humanoid = character:FindFirstChildOfClass("Humanoid")
                    if not humanoid then return end
                    if character:FindFirstChild("InvisRunning") then return end
                    local flag = Instance.new("BoolValue")
                    flag.Name = "InvisRunning"
                    ...
                    anim.AnimationId = INVIS_ANIM
                    local track
                    pcall(function() track = animator:LoadAnimation(anim) end)
                    if not track then flag:Destroy(); return end
                    ...
                    flag:Destroy()
                    pcall(function()
                        loadstring(game:HttpGet(NOTIFY_URL))():SendNotification("Invisible","Invisibility applied successfully",3)
                    end)
                end
...
if not mobile then
   ...
   ModeSection:AddParagraph({Title="WARNING Fallen Parts Height",Content="Do not touch unless you know what you are doing! | Center (0) = NaN/disabled"})
    ...
    ExtrasLeft:AddParagraph({Title="Invisibility 2", Content="Rejoin to disable"})
    ExtrasLeft:AddButton({Name="Apply Invisibility 2", Callback=function()
        task.spawn(function()
            repeat task.wait() until game:IsLoaded()
            local Players = game:GetService("Players")
            local LocalPlayer = Players.LocalPlayer
